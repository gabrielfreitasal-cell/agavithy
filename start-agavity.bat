...
REM ── Start Whisper server in background
echo [1/4] Starting Whisper transcription server...
start "Agavity Whisper" /MIN cmd /c "cd /d %~dp0 && python whisper_server.py --model base --warmup"

REM ── Start API server in background
echo [2/4] Starting API server on port 8080...
...
REM ── Start Electron
echo [4/4] Launching Agavity desktop app...
